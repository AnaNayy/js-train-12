...
// Завдання: 5
/**
 * Функція `getKeysStartingWith` повертає масив ключів словника, які
 * починаються з заданої букви.
...
// Завдання: 6
/**
 * Функція `addKeyValuePairs` додає пари ключ-значення до словника з перевіркою
 * на унікальність ключа.
...
function addKeyValuePairs(dictionary, entries) {
  // Ведемо статистику доданих та відхилених ключів, для цього
  // створемо змінні added та rejected з початковими значеннями 0
...
function transformDictionary(dictionary) {
  // Створюємо порожні масиви для ключів (змінна keys),
  // значень(змінна values) і пар ключ - значення(змінна entries)
...
function checkPresence(dictionary, keys) {
  ...
  keys.forEach((key) => {
    present.push(dictionary.has(key));
  });
  return present;
}
